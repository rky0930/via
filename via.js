...
var _via_default_attribute = "";   // data structure to store loaded images metadata
...
var _via_default_attribute_number      = 0;
...
function sel_local_data_file(type) {
  if (_via_default_attribute == "") {
    show_message('Please Set Default Attribute')
  }else{
    if (invisible_file_input) {
  ...
  for ( var i=0; i < csvdata.length; ++i ) {
    ...
    if (csvdata[i].charAt(0) === VIA_IMPORT_CSV_COMMENT_CHAR) {
      ...
      if ( _via_img_metadata.hasOwnProperty(image_id) ) {
        ...
        // Create file select box for JSON import
        create_default_attribute_select_box('f', 0);
        ...
        if ( d[region_attr_index] !== '"{}"' ) {
          ...
          // Create select box for CSV Import
          if(image_id !== image_id_tmp) {
            image_id_tmp = image_id
            create_region_index = 0; 
          }
          create_default_attribute_select_box('r', create_region_index);
          create_region_index++;
  ...
  for (var image_id in d) {
    if ( _via_img_metadata.hasOwnProperty(image_id) ) {
      ...
      for ( var i in regions ) {
        ...
        // Create region select box for JSON import
        create_default_attribute_select_box('r', i);
...
function show_image(image_index) {
  ...
  img_reader.addEventListener( "load", function() {
    ...
    _via_current_image.addEventListener( "load", function() {
      ...
      // update region select box when showing image is changed
      update_default_attribute_select_box();
      hide_default_attribute_select_box();
...
function show_img_list() {
  ...
  if(_via_is_loaded_img_list_visible && _via_current_image_loaded) {
    ...
    // Init default attirbute (Hard code)
...
function set_region_select_state(region_id, is_selected) {
  // Move default select box to selected region.
  move_default_region_attribute_select_box(region_id) 
...
_via_reg_canvas.addEventListener('dblclick', function(e) {
  ...
  // move file attribute select box
  if (_via_is_file_attr_panel_visible) {
        move_default_attribute_select_box(0, 'f',_via_click_y0, _via_click_x0)
  }
}, false);
...
_via_reg_canvas.addEventListener('mouseup', function(e) {
  ...
  if ( click_dx < VIA_MOUSE_CLICK_TOL ||
    ...
    if ( _via_is_user_drawing_polygon ) {
      ...
      if ( Math.sqrt(dx*dx + dy*dy) <= VIA_POLYGON_VERTEX_MATCH_TOL ) {
        ...
        // Create default select box for POLYGON
        create_default_attribute_select_box('r', _via_canvas_regions.length - 1)
        move_default_region_attribute_select_box(_via_canvas_regions.length - 1)   
      ...
      if ( region_id >= 0 ) {
        ...
        if ( _via_is_user_drawing_region ) {
          ...
          case VIA_REGION_SHAPE.POINT:
            ...
            // Create default select box for POINT
            create_default_attribute_select_box('r', _via_canvas_regions.length - 1)
            move_default_region_attribute_select_box(_via_canvas_regions.length - 1)      
  ...
  if ( _via_is_user_drawing_region ) {
    ...
    // Create default select box for RECT, CIRCLE, ELLIPSE
    create_default_attribute_select_box('r', _via_user_sel_region_id)
    move_default_region_attribute_select_box(_via_user_sel_region_id)
...
window.addEventListener('keydown', function(e) {
  ...
  var dummyEl = document.getElementById('DefaultAttributeTextArea');
  var isFocused = (document.activeElement === dummyEl);
  // user commands
  if(isFocused){
    //DefaultAttributeTextArea has focus
  }
  else{
    if ( e.ctrlKey ) {
...
function del_sel_regions() {
  ...
  // update region select box for pasting  
  update_default_attribute_select_box();
  save_current_data_to_browser_cache();
...
function toggle_file_attr_panel() {
  if ( _via_current_image_loaded ) {
    ...
    // Create default file attribute select box.
    create_default_attribute_select_box('f', 0)
    update_vertical_space();
  } else {
    show_message('Please load some images first');
...
// Yoon Added functions
function init_default_attribute() {
  
  TextAreaValue = document.getElementById("DefaultAttributeTextArea").value
  if (TextAreaValue != "") {
    _via_default_attribute = JSON.parse(TextAreaValue)
  }

  for (var attr in _via_default_attribute) {
    if (_via_default_attribute.hasOwnProperty(attr)) {
        add_new_attribute('r', attr);
        add_new_attribute('f', attr);
    }
  }
  show_message('Set default attribute.');
}

function create_default_attribute_select_box(type, region_id) {
  // clear check box to make new check box 
  clear_default_attribute_select_box(region_id);

  select_box_id = 'sb#'+type+'#'+region_id;
  if(document.getElementById(select_box_id) !== null) {
    // default_attribute_select_box already exist.
    return;
  }

  select_box_class = 'default_attribute_select_box';
  html_attribute_and_value = '';
  
  for (var attribute in _via_default_attribute) {
    if (_via_default_attribute.hasOwnProperty(attribute)) {
      select_box_attribute_id = select_box_id + '#'+attribute;
      html_attribute_and_value += '<div ' +
        ' id="'+select_box_attribute_id+'"' +
        ' style="border-bottom: 0.5px solid #000;">' + attribute + '</div>';

      _via_default_attribute[attribute].forEach( function(value){
        
        checked_fn = 'update_attribute_value(\'' + type + '#' + attribute + '#' + region_id + '\',\'' + value + '\')';
        
        select_box_attribute_input_id = select_box_attribute_id + '#'+ value
        select_box_attribute_input_class_name = 'default_select_box_attribute_input'
        html_attribute_and_value += '<input ' +
        ' id="' + select_box_attribute_input_id + '"' + 
        ' class="' + select_box_attribute_input_class_name + '"' + 
        ' type = "checkbox"' +
        ' onclick="' + checked_fn + '"' +
        ' >' + value + '</input> <br>';
      });
  ...
  html_select_box = '<div ' +
    ' id="' + select_box_id + '"' +
    ' class="' + select_box_class + '"' +
    ' style="top: 30px; left: 30px; display: none"' +
    ' >' + html_attribute_and_value + '</div> ';
  
  document.getElementById("region_canvas").insertAdjacentHTML('afterend', html_select_box);
  _via_default_attribute_number++
}

function clear_default_attribute_select_box_all() {
  default_select_box_attribute_input = document.getElementsByClassName("default_select_box_attribute_input")
  for (var i=0; i<default_select_box_attribute_input.length; i++) {
    default_select_box_attribute_input[i].checked = false;
  }    
}

// clear specific region checkbox
function clear_default_attribute_select_box(region_id) {  
  select_box_id = 'sb#r#' + region_id;
  if(!document.getElementById(select_box_id)) {
    return 
  }
  ...
  for (var attribute in _via_default_attribute) {
    if (_via_default_attribute.hasOwnProperty(attribute)) {
      _via_default_attribute[attribute].forEach( function(value) {
        select_box_attribute_id = select_box_id + '#'+attribute+'#'+value;
        document.getElementById(select_box_attribute_id).checked = false;
      });      
    }
  }
}

function update_default_attribute_select_box() {
  //set all check box false
  clear_default_attribute_select_box_all();
  
  // reload from metadata 
  regions = _via_img_metadata[_via_image_id].regions;
  for (var i=0; i<regions.length; i++) { // all regions
    region = regions[i];

    region.region_attributes.forEach(function (selected_value, key) {  // multi-attribute
      _via_default_attribute[key].forEach( function(value){
        
        select_box_id = 'sb#r#' + i + '#' + key + '#' + value;
        if(selected_value===value) {
          document.getElementById(select_box_id).checked = true;
        } else {
          document.getElementById(select_box_id).checked = false;
        }
      });      
    });
  }

  file_attributes = _via_img_metadata[_via_image_id].file_attributes;
  file_attributes.forEach(function (selected_value, key) {  // multi-attribute
    _via_default_attribute[key].forEach( function(value){
      
      select_box_id = 'sb#f#0#' + key + '#' + value;
      if(selected_value===value) {
        document.getElementById(select_box_id).checked = true;
      } else {
        document.getElementById(select_box_id).checked = false;
...
function move_default_attribute_select_box(id, type, top, left) {
  select_box_id = 'sb#' + type + '#' + id;

  document.getElementById(select_box_id).style.top = top+"px";
  document.getElementById(select_box_id).style.left = left+"px";
  
  hide_default_attribute_select_box();
  document.getElementById(select_box_id).style.display = 'inline';
}

function move_default_region_attribute_select_box(region_id) {
  select_box_id = 'sb#r#' + region_id;
  attr = _via_canvas_regions[region_id].shape_attributes;
  
  switch(attr.get("name")){ 
    case "rect":
      most_right  = attr.get("x") + attr.get("width");
      most_bottom = attr.get("y") + attr.get("height");
      break;
    case "circle":
      most_right  = attr.get("cx") + attr.get("r");
      most_bottom = attr.get("cy") + attr.get("r");
      break;
    case "ellipse":
      most_right  = attr.get("cx") + attr.get("rx");
      most_bottom = attr.get("cy") + attr.get("ry");
      break;
    case "polygon":       
      x_array = attr.get("all_points_x");
      y_array = attr.get("all_points_y");
      max_index = x_array.indexOf(Math.max.apply(null, x_array));      
      most_right  = x_array[max_index];
      most_bottom = y_array[max_index];
      break;
    case "point":
      most_right  = attr.get("cx");
      most_bottom = attr.get("cy");
      break ;     
  }
  move_default_attribute_select_box(region_id, 'r', most_bottom, most_right);
}

function hide_default_attribute_select_box() {
  select_boxes = document.getElementsByClassName("default_attribute_select_box");
  for (var i=0; i<select_boxes.length; i++) {
    select_boxes[i].style.display = 'None';
  }
}

function show_default_attribute(e) {
  e.classList.toggle('active');
  e.nextElementSibling.classList.toggle('show');
  if (_via_default_attribute != ""){ 
    document.getElementById('DefaultAttributeTextArea').value = JSON.stringify(_via_default_attribute, undefined, 2);  
  }
}

function recreate_default_attribute_select_box() {
  default_attribute_select_boxs = document.getElementsByClassName("default_attribute_select_box")
  for (var i=0, box; box =  default_attribute_select_boxs[i]; i++) {
    box_id = box.getAttribute('id');
    type_and_region_id = box_id.split("#");
    box.remove();
    type = type_and_region_id[1]
    region_id = type_and_region_id[2]
    create_default_attribute_select_box(type, region_id)
  }
  // update default attribute select box
  if (default_attribute_select_boxs.length > 0) {
    update_default_attribute_select_box()
  }
}

function set_default_attribute() {
  default_attribute_text = document.getElementById('DefaultAttributeTextArea').value
  if (default_attribute_text == "") {
    _via_default_attribute = {}
  } else {
    _via_default_attribute = JSON.parse(default_attribute_text)
    // Format text to Json 
    document.getElementById('DefaultAttributeTextArea').value = JSON.stringify(_via_default_attribute, undefined, 2);
  } 
  recreate_default_attribute_select_box()
}

function default_attribute_textarea_change() {
  var DefaultAttributeExampleSelectBox = document.getElementById('DefaultAttributeExampleSelectBox')
  if (DefaultAttributeExampleSelectBox.value != 'none') {
    DefaultAttributeExampleSelectBox.selectedIndex = 0;
  } 
}

function default_attribute_example_selectbox_change() {
  var selected_value = document.getElementById('DefaultAttributeExampleSelectBox').value
  if (selected_value == 'none') {
    document.getElementById('DefaultAttributeTextArea').value = ""
  } else {
    if (selected_value == 'mscoco') {
      _via_default_attribute = { 
        "display_name":  
    ...
    } else if (selected_value == 'kitti') {
      _via_default_attribute = { 
        "name": ["car", "pedestrian"] 
      }
    } else if (selected_value == 'pascal') {
      _via_default_attribute = { 
        "display_name":  
...
//function _via_hook_prev_image() {}    </script>
